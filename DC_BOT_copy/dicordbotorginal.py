...
async def on_message(message):
    ...
    if message.content.startswith('HELLO LORD'):
        ...
        for attachment in message.attachments:
            ...
            img_class = detect_arduino(model_path="DC_BOT_copy\keras_model.h5", labels_path="DC_BOT_copy\labels.txt",image_path=f"./{attachment.filename}")
            print(img_class[0])
            if img_class.strip() == 'ARDUINO UNO':
                await message.channel.send("This is an Arduino UNO board.")
            elif img_class.strip() == 'ARDUINO NANO':
                await message.channel.send("This is an Arduino Nano board.")
            elif img_class.strip() == 'ARDUINO MEGA':
                await message.channel.send("This is an Arduino Mega board.")
            if not message.attachments:
                await message.channel.send("I have no idea what this is.")

    else:
        await message.attachments(message.content)
        



client.run("ODI4MTg2MTA4NzU1MDUwNTI2.Gvr3aK.CgbkfauL-NU3Ol9rs8hy7xfLOcNIEKYlRoBMIk")